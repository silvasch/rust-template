...
fn load_dotenv() -> Result<(), Error> {
    ...
    tracing::info!(target: TARGET, "loading a .env file");

    match dotenvy::dotenv() {
        Ok(file_path) => {
            tracing::info!(target: TARGET, "successfully loaded {}", file_path.display());
        }
        Err(dotenvy::Error::Io(e)) if e.kind() == std::io::ErrorKind::NotFound => {
            tracing::info!(target: TARGET, "could not found a .env file; skipping");
        }
